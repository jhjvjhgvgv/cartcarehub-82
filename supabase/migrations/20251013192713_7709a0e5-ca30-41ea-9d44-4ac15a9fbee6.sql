...
-- 1. Create role enum
CREATE TYPE public.app_role AS ENUM ('admin', 'maintenance', 'store');
...
-- 7. Update existing functions to use new role system
...
-- 9. Update trigger for new users to use user_roles
...
-- 10. Comment: Keep role column in profiles for backward compatibility during migration
-- We'll deprecate it later after updating all client code
